...
const chromiumFlags: any[] = [
    ...
    // FPS解放周り
    ['disable-frame-rate-limit', null, config.get('graphics.UnlimitedFPS', true)],
    ['disable-gpu-vsync', null, config.get('graphics.UnlimitedFPS', true)],
    // 描画関係
    ['use-angle', config.get('graphics.AngleType', 'default'), true],
    ['enable-webgl2-compute-context', null, config.get('graphics.GL2Context', true)],
    ['disable-accelerated-2d-canvas', 'true', !config.get('graphics.HWAcceleration', true)],
    // ウィンドウキャプチャに必要な設定(win32でのみ動作する。frznさんに感謝)
    ['in-process-gpu', null, process.platform === 'win32' ? true : false],
    // その他
    ['autoplay-policy', 'no-user-gesture-required', config.get('general.AutoPlay', true)],
];
